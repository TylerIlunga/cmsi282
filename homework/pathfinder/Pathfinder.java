...
/**
 * Maze Pathfinding algorithm that implements the informed search strategy
 * A* star graph search
 */
public class Pathfinder {
    ...
    public static ArrayList<String> solve (MazeProblem problem, MazeState initState, Set<MazeState> objectives) {
        ...
        while(!frontier.isEmpty()) {
            ...
            if (objectives.contains(current.state)) { return getActionSequence(current);}
                ...
                if (!closedSet.contains(nextState)) {
                  frontier.add(
                    new SearchTreeNode(
                      ...
                      nextAction,
                      current,
                      current.totalCost + problem.getCost(nextState),
                      computeHeuristic(nextState, objectives)
                    )
                  );
                }
            }
    ...
    /**
     * Returns the sum of the absolute values of the differences of the given
     * state coordinates.
     *
    ...
    /**
     * Returns the heuristic cost to closet goal, from a given state, within a
     * list of objectives.
     *
...
class SearchTreeNode {
    ...
    public String toString() {
      return String.format (
          "Node:\nState: %s\nAction: %s\nParent: %s\ng(n): %d\nh(n): %d, f(n): %d",
          this.state, this.action, this.parent,
          this.totalCost, this.heuristic, this.evaluation
      );
    }
}
